...
app.put('/users/:email/role', authenticateToken, checkRole('admin'), async (req, res) => {
  ...
  app.post('/tasks', authenticateToken,checkRole('admin'), async (req, res) => {
    try {
        ...
        db.run(`INSERT INTO Tasks (title, description, status, assigned_user_id) VALUES (?, ?, ?, ?)`, [title, description, status, assigned_user_id], function(err) {
            if (err) {
 ...
 */
  ...
  app.put('/tasks/:id', authenticateToken,checkRole('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, status, assigned_user_id } = req.body;
        db.run(`UPDATE Tasks SET title = ?, description = ?, status = ?, assigned_user_id = ? WHERE id = ?`, [title, description, status, assigned_user_id, id], function(err) {
            if (err) {
                return res.status(400).json({ error: err.message });
            }
